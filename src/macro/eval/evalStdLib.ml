...
module StdCompress = struct
	...
	let exec (f : zfun) vthis src srcPos dst dstPos =
		let this = this vthis in
		let src_bytes = decode_bytes src in
		let src = Bytes.unsafe_to_string src_bytes in
		let spos = decode_int srcPos in
		let slen = Bytes.length src_bytes - spos in
		let dst = decode_bytes dst in
		let dpos = decode_int dstPos in
		let dlen = Bytes.length dst - dpos in
		let r = try f this.z ~src ~spos ~slen ~dst ~dpos ~dlen this.z_flush with _ -> exc_string "oops" in
		encode_obj [
